...
$sel_lineas="SELECT * FROM visiteurs  where (date='$favis1')";    
$rs_lineas=mysql_query($sel_lineas);
for ($i = 0; $i < mysql_num_rows($rs_lineas); $i++) {
 $firma=mysql_result($rs_lineas,$i,"firma");
}
...
//$pdf->Image('$firma',60,30,90,0,'PNG');

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//$firma=

 $pdf->Cell( 13, 7, $pdf->Image($firma, $pdf->GetX(), $pdf->GetY(), 6.78), 0, 0, 'L', false );


 //$pdf->Image($firma, '', '', 30, 10, '', '', 'T', false, 300, '', false, false, 1, false, false, false);
 

//$pdf->Cell(13,7,$pdf->Image($firma['SRC'], $pdf->GetX()+2, $pdf->GetY()+2, 20.78),0,0,'',true);
// $pdf->Cell(13,7," ",1,0); //** 
